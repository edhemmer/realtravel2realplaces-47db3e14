/**
 * NowCard renders only evidence-backed states from useNowCard.
 * Command-like airport timing and assumed-location fallback states stay hidden
 * until the underlying engine has route/location/freshness evidence.
 */
...
const PRESSURE_META: Record<NowCardPressure, { Icon: typeof Radio; tagText: string; tone: string }> = {
  ...
  'airport-buffer': { Icon: Plane, tagText: 'Flight window', tone: 'text-warning' },
...
export function NowCard({ trip, className }: { trip: Trip | null; className?: string }) {
  ...
  const isUnsupportedCommandState =
    card.pressure === 'airport-buffer' ||
    card.pressure === 'departure-day' ||
    card.shapeKey === 'hydrating';

  if (!trip || card.pressure === 'idle' || !card.headline || isUnsupportedCommandState) return null;
    ...
    <AnimatePresence mode="wait">
      ...
      >
        <GlassSurface elevation="floating" className="overflow-hidden">
          <div
            aria-hidden
            className={`pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/60 to-transparent ${card.pressure === 'post-trip' ? 'via-success/60' : ''}`}
          />
          <div
            aria-hidden
            className={`pointer-events-none absolute inset-x-0 top-0 h-12 bg-gradient-to-b ${card.pressure === 'post-trip' ? 'from-success/8' : 'from-primary/8'} to-transparent`}
          />
          ...
          <div className="relative py-4 px-4 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[15px] font-semibold leading-snug truncate mt-0.5">{card.headline}</p>
                {card.subtext && <p className="text-[12px] text-muted-foreground mt-0.5 truncate">{card.subtext}</p>}
              </div>
            </div>
